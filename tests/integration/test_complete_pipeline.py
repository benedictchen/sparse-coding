...
from sparse_coding import (
    SparseCoder, DictionaryLearner, AdvancedOptimizer, L1Proximal,
    TB, CSVDump, DashboardLogger, visualization
)
from sparse_coding.whitening import zero_phase_whiten
...
class TestBasicPipelineIntegration:
    ...
    def test_advanced_optimization_integration(self, synthetic_data):
        """Test integration with advanced optimization methods."""
        data = synthetic_data
...
class TestVisualizationIntegration:
    ...
    def test_comprehensive_report_integration(self, natural_image_patches):
        """Test integration with comprehensive analysis report."""
        # Prepare data
        patch_size = int(np.sqrt(natural_image_patches.shape[0]))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            report_path = visualization.create_comprehensive_report(
                dictionary=D,
                sparse_codes=A,
                signals=X,

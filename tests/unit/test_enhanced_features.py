"""
Tests for enhanced sparse coding features from implementation comparison.

Tests the new features incorporated based on comparison analysis:
- Lambda annealing
- paper_gdD mode with gradient dictionary updates
- Homeostatic equalization
- Enhanced NCG with Polak-Ribière
- Improved dead atom detection
"""
...
class TestEnhancedNCG:
    """Test enhanced NCG implementation."""
...
class TestImprovedDeadAtomHandling:
    """Test improved dead atom detection and handling."""
...
class TestResearchAccuracy:
    ...
    def test_enhanced_features_maintain_mathematical_properties(self, synthetic_data):
        """Test that enhanced features maintain mathematical correctness."""
        data = synthetic_data
        X = data['signals'][:, :10]
        n_atoms = min(data['n_components'], X.shape[1] - 2)

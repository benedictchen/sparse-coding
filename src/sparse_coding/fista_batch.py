...
def fista_batch(D, X, lam, L=None, max_iter=200, tol=1e-6, return_objectives=False):
    """
    ...
    Args:
        ...
        return_objectives: Return objective history for convergence analysis
        
    Returns:
        A: Sparse codes matrix (n_atoms, n_samples)
        objectives: (optional) List of objective values if return_objectives=True
    ...
    objectives = [] if return_objectives else None
    ...
    for iteration in range(max_iter):
        ...
        if return_objectives:
            residual = X - D @ A_next
            data_fit = 0.5 * np.sum(residual * residual)
            sparsity = lam * np.sum(np.abs(A_next))
            objectives.append(data_fit + sparsity)
